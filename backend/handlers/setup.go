...
// SyncGoogleProSessionHandler syncs the host machine's active Google AI Pro session (~/.gemini/oauth_creds.json) to Daytona Sandbox
func SyncGoogleProSessionHandler(daytonaSvc *services.DaytonaService, agySvc *services.AGYService) gin.HandlerFunc {
	return func(c *gin.Context) {
		var req struct {
			ApiKey    string `json:"apiKey"`
			ServerUrl string `json:"serverUrl,omitempty"`
		...
		email, err := agySvc.SyncLocalGoogleCredentials(req.ApiKey, req.ServerUrl, req.SandboxID)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to sync local Google AI Pro session: " + err.Error()})
			return
		}

		c.JSON(http.StatusOK, gin.H{
			"success": true,
			"message": "Google AI Pro subscription session successfully synced to Daytona sandbox volume!",
			"email":   email,
		})
	}
}

// ImportGoogleOAuthJSONHandler allows direct paste of Google OAuth JSON into Daytona persistent volume
func ImportGoogleOAuthJSONHandler(daytonaSvc *services.DaytonaService, agySvc *services.AGYService) gin.HandlerFunc {
	return func(c *gin.Context) {
		var req struct {
			ApiKey    string `json:"apiKey"`
			ServerUrl string `json:"serverUrl,omitempty"`
		...
		if err := agySvc.ImportGoogleOAuthJSON(req.ApiKey, req.ServerUrl, req.SandboxID, req.JSON, req.Email); err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to import Google OAuth credentials: " + err.Error()})
			return
		}

		c.JSON(http.StatusOK, gin.H{
			"success": true,
			"message": "Google OAuth session imported into Daytona persistent volume!",
		})
	}
}
