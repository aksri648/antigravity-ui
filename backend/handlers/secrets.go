...
// SaveSecretsHandler updates cloud integration secrets in SQLite and syncs to Daytona Volume .env
func SaveSecretsHandler(daytonaSvc *services.DaytonaService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Sync directly to Daytona Volume .env if sandbox is active
...
`, envContent, envContent)
		...
		c.JSON(http.StatusOK, gin.H{
			"success": true,
			"message": "Cloud integration secrets saved and synchronized to persistent Daytona volume.",
		})
	}
}
