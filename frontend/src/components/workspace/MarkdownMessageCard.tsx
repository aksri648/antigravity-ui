...
export const MarkdownMessageCard: React.FC<MarkdownMessageCardProps> = ({
  ...
  if (message.sender === "user") {
    return (
      <div className="w-full flex justify-end group">
        <div className="max-w-[85%] rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-600 text-white p-3.5 shadow-lg shadow-blue-900/20 border border-blue-400/20 space-y-1.5">
          <div className="flex items-center justify-between gap-2 text-[10px] text-blue-100 font-medium pb-1 border-b border-blue-400/20">
            ...
            <button
              ...
              title="Copy message"
            >
              {copiedMessage ? <Check className="h-3 w-3 text-emerald-200" /> : <Copy className="h-3 w-3" />}
            </button>
          </div>
          <p className="text-xs leading-relaxed whitespace-pre-wrap font-sans selection:bg-blue-300 selection:text-black">
            {message.text}
          </p>
        </div>
      </div>
  ...
  return (
    <div className="w-full rounded-2xl bg-[#141418] border border-white/10 shadow-xl overflow-hidden group transition-all hover:border-white/20">
      {/* Assistant Card Header */}
      <div className="px-4 py-2.5 bg-[#1a1a20]/90 border-b border-white/5 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-white tracking-wide">DELTA Agent</span>
          <Badge
            variant="outline"
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            size="sm"
            onClick={handleCopyMessage}
            className="h-6 w-6 p-0 text-muted-foreground hover:text-white rounded-md hover:bg-white/10"
      ...
      <div className="p-4 text-xs text-gray-200 leading-relaxed space-y-3 font-sans selection:bg-emerald-500/30 selection:text-white">
        {!message.text && isProcessing ? (
          <div className="flex items-center gap-2 text-muted-foreground text-xs italic py-2">
            <span className="h-2 w-2 rounded-full bg-emerald-400 animate-ping" />
            Thinking & formulating plan...
          </div>
        ) : (
          <ReactMarkdown
            ...
            components={{
              // Headings
              h1: ({ children }) => (
                <h1 className="text-base font-bold text-white border-b border-white/10 pb-1.5 mt-3 mb-2 flex items-center gap-2">
                  <span className="h-2 w-2 rounded-full bg-emerald-400" />
                  {children}
                </h1>
              ),
              h2: ({ children }) => (
                <h2 className="text-sm font-bold text-emerald-300 border-b border-white/5 pb-1 mt-3 mb-1.5">
                  {children}
                </h2>
              ),
              h3: ({ children }) => (
                <h3 className="text-xs font-bold text-blue-300 mt-2.5 mb-1">{children}</h3>
              ),
              h4: ({ children }) => (
